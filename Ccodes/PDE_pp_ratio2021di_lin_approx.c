...
#include "./global_params_pp2021_lin_approx.c"
...
{	
  ...
  //fp1 = fopen("av_lam_data_alf=2.0e-4_v3.csv", "r");
  fp1 = fopen("av_lam_data_lin_v4.csv", "r");
  //fp1 = fopen("av_lam_data_lin_v5.2.csv", "r");
...
#ifdef MONOD_ZERO
  for(k = 1; k <= NUM_LAYER; k++) {
    v[jP[k]] = (1.0 + (a0/d0)*v[jR[k]])*v[jP[k]];
  }	
#endif	
...
double dPkdt(double time, double vr[], int k)
{
  ...
  temp1 /= (1.0 + (a0/d0)*vr[jR[k]]*av_cont_diff(matrix_lambda, interval_discontinuous, interval_differential, lammda_k)); 
...
#ifdef  MONOD_ZERO
  temp = h1*(a0/d0)*resource*predator/(1.0 + (a0/d0)*resource) + mR*resource;
#endif	
...
double growthP(double resource, double predator, int k)
{
  ...
  //normal MONOD equation for B_T and A_T
#ifdef MONOD_ZERO
  temp = (gB - mR)*(a0/d0)*resource/(1.0 + (a0/d0)*resource) - mB*pow(predator, dellta2);
  temp *= predator;
#endif	
...
#ifdef AGG
  temp = 1.0/(1.0 + (a0/d0)*resource*av_cont_diff(matrix_lambda, interval_discontinuous, interval_differential, lambda_ap));
